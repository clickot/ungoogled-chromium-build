...
if [ -z "$(docker images -q ${IMAGE})" ] ; then
    ...
    sudo rm -rf ${GIT_REPO}
    docker build -t ${IMAGE} --build-arg DISTRO=${DISTRO} --build-arg RELEASE=${RELEASE} --build-arg LLVM_VERSION=${LLVM_VERSION} --build-arg LLVM_FULL_VERSION=${LLVM_FULL_VERSION} --build-arg REPO_POSTFIX=${REPO_POSTFIX} .
else
